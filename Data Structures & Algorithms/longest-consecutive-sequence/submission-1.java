class Solution {
    public int longestConsecutive(int[] nums) {
        ...
        for(int i=0;i<nums.length;i++){
            ...
            // instead of using ele use the count suppose 2 is the start we have [2,3,4,5]
            // when we check if 3 is present we will increase the count to 1 
            // 2+1 -> 3
            // for 4 the count will be 2 
            // 2+2 -> 4 
            // for 5 the count will be 3
            // 2+3 -> 5 
            // use the sequence instead of the creating a new element
            // int ele = nums[i];
            ...
            if(!mapOfNums.containsKey(nums[i]-1)){
                ...
                while(mapOfNums.containsKey(nums[i]+length) && mapOfNums.get(nums[i]+length)==0){
                    mapOfNums.put(nums[i]+length,1);
                    length++;
